...
const CardHeader = ({ title, subtitle, mode = 'light' }: { title: string; subtitle?: string; mode?: 'light' | 'dark' }) => {
...
const CardBody = ({ children, mode = 'light' }: { children: React.ReactNode; mode?: 'light' | 'dark' }) => {
  const c = mode === 'light' ? colors : colorsDark;
  return (
    <div style={{
      ...
      <section style={{ marginBottom: '48px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '32px' }}>
          <Card variant="raised" mode={mode}>
            <CardHeader title="Raised Card" subtitle="Default neumorphic style" mode={mode} />
            <CardBody mode={mode}>
              This card appears to float above the surface with soft shadows on both sides.
            </CardBody>
          </Card>

          <Card variant="flat" mode={mode}>
            <CardHeader title="Flat Card" subtitle="Minimal style" mode={mode} />
            <CardBody mode={mode}>
          ...
          <Card variant="cut" mode={mode}>
            <CardHeader title="Cut Card" subtitle="Inset/pressed style" mode={mode} />
            <CardBody mode={mode}>
              This card appears to be cut into the surface, creating a recessed effect.
            </CardBody>
          </Card>

          <Card variant="bordered" mode={mode}>
            <CardHeader title="Bordered Card" subtitle="With subtle border" mode={mode} />
            <CardBody mode={mode}>
      ...
      <section>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '32px' }}>
          <Card variant="raised" interactive mode={mode}>
            <CardHeader title="Interactive Raised" subtitle="Hover for glow effect" mode={mode} />
            <CardBody mode={mode}>
          ...
          <Card variant="cut" interactive mode={mode}>
            <CardHeader title="Interactive Cut" subtitle="Click to press deeper" mode={mode} />
            <CardBody mode={mode}>
          ...
          <Card variant="bordered" interactive mode={mode}>
            <CardHeader title="Interactive Bordered" subtitle="Hover for rim glow" mode={mode} />
            <CardBody mode={mode}>
              The terracotta rim glows on hover.
            </CardBody>
          </Card>
        </div>
      </section>
  ...
  return (
    <div style={{ padding: '32px', backgroundColor: bgColor, minHeight: '100vh' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '24px' }}>
        {items.map((item) => (
          <Card key={item.title} variant="raised" size="md" interactive mode={mode}>
...
export const Playground: Story = {
  ...
  render: (args) => (
    <div style={{ padding: '32px', backgroundColor: args.mode === 'light' ? '#FDF8F4' : '#1C1917' }}>
      <Card {...args}>
        <CardHeader title="Card Title" subtitle="Card subtitle" mode={args.mode} />
